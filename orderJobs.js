const orderJobs = (jobDependencies) => {
    ...
    }).sort() // then sorts job dependencies into alphabetical order to overcome false circular dependencies

    const nonPriorityJobsArray = formattedJobDependenciesArray.reduce((nonPriorityJobsAcc, jobDependency) => { // creates array of jobs that have no dependencies
        return jobDependency.length === 1 ? [...nonPriorityJobsAcc, jobDependency] : nonPriorityJobsAcc // if job has no dependency add to nonPriority jobs
    }, [])

    const jobSequenceArray = formattedJobDependenciesArray.reduce((jobSequenceAcc, jobDependency) => {
        if (jobDependency.length === 1) { // if no jobDependency ie.e non priority job, ignore and return jobSequenceArr
            ...
            if (priorityJob === nonPriorityJob) { // if jobs are the same i.e. self dependent
                ...
                return [...jobSequenceAcc, priorityJob, nonPriorityJob] // adds job pair to end of array
